webpackJsonp([4],{

/***/ 30:
/***/ function(module, exports) {

	module.exports = "data:image/png;base64,"

/***/ },

/***/ 32:
/***/ function(module, exports) {

	module.exports = "data:image/png;base64,"

/***/ },

/***/ 82:
/***/ function(module, exports, __webpack_require__) {

	var __vue_exports__, __vue_options__
	var __vue_styles__ = {}

	/* styles */
	__webpack_require__(83)

	/* script */
	__vue_exports__ = __webpack_require__(85)

	/* template */
	var __vue_template__ = __webpack_require__(86)
	__vue_options__ = __vue_exports__ = __vue_exports__ || {}
	if (
	  typeof __vue_exports__.default === "object" ||
	  typeof __vue_exports__.default === "function"
	) {
	if (Object.keys(__vue_exports__).some(function (key) { return key !== "default" && key !== "__esModule" })) {console.error("named exports are not supported in *.vue files.")}
	__vue_options__ = __vue_exports__ = __vue_exports__.default
	}
	if (typeof __vue_options__ === "function") {
	  __vue_options__ = __vue_options__.options
	}
	__vue_options__.__file = "/Users/lvyang/repositories/node_web/components/no_content.vue"
	__vue_options__.render = __vue_template__.render
	__vue_options__.staticRenderFns = __vue_template__.staticRenderFns

	/* hot reload */
	if (false) {(function () {
	  var hotAPI = require("vue-loader/node_modules/vue-hot-reload-api")
	  hotAPI.install(require("vue"), false)
	  if (!hotAPI.compatible) return
	  module.hot.accept()
	  if (!module.hot.data) {
	    hotAPI.createRecord("data-v-3b285777", __vue_options__)
	  } else {
	    hotAPI.reload("data-v-3b285777", __vue_options__)
	  }
	})()}
	if (__vue_options__.functional) {console.error("[vue-loader] no_content.vue: functional components are not supported and should be defined in plain js files using render functions.")}

	module.exports = __vue_exports__


/***/ },

/***/ 83:
/***/ function(module, exports, __webpack_require__) {

	// style-loader: Adds some css to the DOM by adding a <style> tag

	// load the styles
	var content = __webpack_require__(84);
	if(typeof content === 'string') content = [[module.id, content, '']];
	// add the styles to the DOM
	var update = __webpack_require__(9)(content, {});
	if(content.locals) module.exports = content.locals;
	// Hot Module Replacement
	if(false) {
		// When the styles change, update the <style> tags
		if(!content.locals) {
			module.hot.accept("!!../node_modules/css-loader/index.js!../node_modules/vue-loader/lib/style-rewriter.js?id=data-v-3b285777!../node_modules/vue-loader/lib/selector.js?type=styles&index=0!./no_content.vue", function() {
				var newContent = require("!!../node_modules/css-loader/index.js!../node_modules/vue-loader/lib/style-rewriter.js?id=data-v-3b285777!../node_modules/vue-loader/lib/selector.js?type=styles&index=0!./no_content.vue");
				if(typeof newContent === 'string') newContent = [[module.id, newContent, '']];
				update(newContent);
			});
		}
		// When the module is disposed, remove the <style> tags
		module.hot.dispose(function() { update(); });
	}

/***/ },

/***/ 84:
/***/ function(module, exports, __webpack_require__) {

	exports = module.exports = __webpack_require__(8)();
	// imports


	// module
	exports.push([module.id, "\n.no_content{\n\twidth: 10rem;\n\theight: 100%;\n\ttext-align: center;\n}\n.no_content div{\n\tmargin-top: 0.3rem;\n\tcolor: #d8d8d8;\n}\n.no_content img{\n\twidth: 3rem;\n\tmargin-top: 4rem;\n}\n", ""]);

	// exports


/***/ },

/***/ 85:
/***/ function(module, exports) {

	"use strict";

	Object.defineProperty(exports, "__esModule", {
		value: true
	});
	//
	//
	//
	//
	//
	//

	exports.default = {
		data: function data() {
			return {};
		},

		props: ["message"]
	};

/***/ },

/***/ 86:
/***/ function(module, exports, __webpack_require__) {

	module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
	  return _c('div', {
	    staticClass: "no_content"
	  }, [_c('img', {
	    attrs: {
	      "src": __webpack_require__(87)
	    }
	  }), _vm._v(" "), _c('div', [_vm._v(_vm._s(_vm.message))])])
	},staticRenderFns: []}
	if (false) {
	  module.hot.accept()
	  if (module.hot.data) {
	     require("vue-loader/node_modules/vue-hot-reload-api").rerender("data-v-3b285777", module.exports)
	  }
	}

/***/ },

/***/ 87:
/***/ function(module, exports, __webpack_require__) {

	module.exports = __webpack_require__.p + "b795755fbb3c417ded3f4ac591fdcfd9.png";

/***/ },

/***/ 193:
/***/ function(module, exports, __webpack_require__) {

	var __vue_exports__, __vue_options__
	var __vue_styles__ = {}

	/* styles */
	__webpack_require__(194)

	/* script */
	__vue_exports__ = __webpack_require__(196)

	/* template */
	var __vue_template__ = __webpack_require__(197)
	__vue_options__ = __vue_exports__ = __vue_exports__ || {}
	if (
	  typeof __vue_exports__.default === "object" ||
	  typeof __vue_exports__.default === "function"
	) {
	if (Object.keys(__vue_exports__).some(function (key) { return key !== "default" && key !== "__esModule" })) {console.error("named exports are not supported in *.vue files.")}
	__vue_options__ = __vue_exports__ = __vue_exports__.default
	}
	if (typeof __vue_options__ === "function") {
	  __vue_options__ = __vue_options__.options
	}
	__vue_options__.__file = "/Users/lvyang/repositories/node_web/views/orders.vue"
	__vue_options__.render = __vue_template__.render
	__vue_options__.staticRenderFns = __vue_template__.staticRenderFns

	/* hot reload */
	if (false) {(function () {
	  var hotAPI = require("vue-loader/node_modules/vue-hot-reload-api")
	  hotAPI.install(require("vue"), false)
	  if (!hotAPI.compatible) return
	  module.hot.accept()
	  if (!module.hot.data) {
	    hotAPI.createRecord("data-v-fe0602da", __vue_options__)
	  } else {
	    hotAPI.reload("data-v-fe0602da", __vue_options__)
	  }
	})()}
	if (__vue_options__.functional) {console.error("[vue-loader] orders.vue: functional components are not supported and should be defined in plain js files using render functions.")}

	module.exports = __vue_exports__


/***/ },

/***/ 194:
/***/ function(module, exports, __webpack_require__) {

	// style-loader: Adds some css to the DOM by adding a <style> tag

	// load the styles
	var content = __webpack_require__(195);
	if(typeof content === 'string') content = [[module.id, content, '']];
	// add the styles to the DOM
	var update = __webpack_require__(9)(content, {});
	if(content.locals) module.exports = content.locals;
	// Hot Module Replacement
	if(false) {
		// When the styles change, update the <style> tags
		if(!content.locals) {
			module.hot.accept("!!../node_modules/css-loader/index.js!../node_modules/vue-loader/lib/style-rewriter.js?id=data-v-fe0602da!../node_modules/vue-loader/lib/selector.js?type=styles&index=0!./orders.vue", function() {
				var newContent = require("!!../node_modules/css-loader/index.js!../node_modules/vue-loader/lib/style-rewriter.js?id=data-v-fe0602da!../node_modules/vue-loader/lib/selector.js?type=styles&index=0!./orders.vue");
				if(typeof newContent === 'string') newContent = [[module.id, newContent, '']];
				update(newContent);
			});
		}
		// When the module is disposed, remove the <style> tags
		module.hot.dispose(function() { update(); });
	}

/***/ },

/***/ 195:
/***/ function(module, exports, __webpack_require__) {

	exports = module.exports = __webpack_require__(8)();
	// imports


	// module
	exports.push([module.id, "\n.orders_content{\n\tbackground-color: #ffffff;\n\theight: 4.3rem;\n\tbox-sizing: border-box;\n\tpadding-left: 0.2rem;\n\tpadding-right: 0.2rem;\n\tmargin-top: 0.2rem;\n}\n.orders_content .orders_code{\n\tbackground: url(" + __webpack_require__(30) + ") 0.2rem center no-repeat;\n\tbackground-size: 0.4rem auto;\n\theight: 1rem;\n\tline-height: 1rem;\n\tbox-sizing: border-box;\n\tpadding-left: 0.7rem;\n\tcolor: #333333;\n\tfont-size: 0.35rem;\n\tborder-bottom: 1px solid #dddddd;\n}\n.orders_content .order_state{\n\tdisplay: inline-block;\n\tcolor: #d81e06;\n\tfloat: right;\n\tbox-sizing: border-box;\n\tpadding-right: 0.3rem;\n}\n.orders_content .orders_commodity{\n\theight: 2rem;\n\tbox-sizing: border-box;\n\tpadding-left: 0.2rem;\n\tpadding-right: 0.2rem;\n\tborder-bottom: solid 1px #dddddd;\n}\n.orders_commodity img{\n\twidth: 1.6rem;\n\tmargin-top: 0.2rem;\n}\n.orders_operation a{\n\tdisplay: inline-block;\n\twidth: 2rem;\n\theight: 0.7rem;\n\ttext-align: center;\n\tline-height: 0.7rem;\n\tfloat: right;\n\tmargin-right: 0.2rem;\n\ttext-decoration: none;\n\tmargin-top: 0.3rem;\n\tborder-radius: 5px;\n\tcolor: #333333;\n\tborder: solid 1px #333333;\n}\n.orders_operation a:nth-child(1){\n\tcolor: #d81e06;\n\tborder: solid 1px #d81e06;\n}\n.orders_bar{\n\tbackground-color: #ffffff;\n\tfont-size: 0px;\n\theight: 1.1rem;\n\tline-height: 1.1rem;\n\tborder-bottom: solid 1px #dddddd;\n\tbox-sizing: border-box;\n}\n.orders_bar a{\n\tbox-sizing: border-box;\n\tdisplay: inline-block;\n\twidth: 20%;\n\theight: 100%;\n\ttext-align: center;\n\tcolor: #333333;\n\tfont-size: 0.35rem;\n\ttext-decoration: none;\n}\n.orders_type{\n\tborder-bottom: solid 2px #1296db;\n\tcolor: #1296db !important;\n}\n", ""]);

	// exports


/***/ },

/***/ 196:
/***/ function(module, exports, __webpack_require__) {

	"use strict";

	Object.defineProperty(exports, "__esModule", {
		value: true
	});

	var _no_content = __webpack_require__(82);

	var _no_content2 = _interopRequireDefault(_no_content);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	exports.default = {
		data: function data() {
			return {
				orders: []
			};
		},

		methods: {
			getOrders: function getOrders() {
				var type = this.$route.params.type;
				if (type == 1) {
					this.orders = [{
						orderId: "",
						orderCode: "123456798",
						orderState: "2",
						orderNum: "2",
						orderPrice: "23.45",
						commodityList: [{
							commodityUrl: "img/logo.jpg"
						}]
					}, {
						orderId: "",
						orderCode: "123456798",
						orderState: "2",
						orderNum: "2",
						orderPrice: "23.45",
						commodityList: [{
							commodityUrl: "img/logo.jpg"
						}]
					}];
				} else {
					this.orders = [];
				}
			}
		},
		watch: {
			'$route': function $route(to, from) {
				this.getOrders();
			}
		},
		mounted: function mounted() {
			this.getOrders();
		},

		components: {
			"jf-no-content": _no_content2.default
		}
	}; //
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//

/***/ },

/***/ 197:
/***/ function(module, exports, __webpack_require__) {

	module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
	  return _c('div', [_c('div', {
	    staticClass: "orders_bar"
	  }, [_c('router-link', {
	    attrs: {
	      "to": "/orders/1",
	      "replace": "",
	      "active-class": "orders_type"
	    }
	  }, [_vm._v("全部")]), _vm._v(" "), _c('router-link', {
	    attrs: {
	      "to": "/orders/2",
	      "replace": "",
	      "active-class": "orders_type"
	    }
	  }, [_vm._v("待付款")]), _vm._v(" "), _c('router-link', {
	    attrs: {
	      "to": "/orders/3",
	      "replace": "",
	      "active-class": "orders_type"
	    }
	  }, [_vm._v("待发货")]), _vm._v(" "), _c('router-link', {
	    attrs: {
	      "to": "/orders/4",
	      "replace": "",
	      "active-class": "orders_type"
	    }
	  }, [_vm._v("待收货")]), _vm._v(" "), _c('router-link', {
	    attrs: {
	      "to": "/orders/5",
	      "replace": "",
	      "active-class": "orders_type"
	    }
	  }, [_vm._v("待评价")])], 1), _vm._v(" "), _c('jf-no-content', {
	    directives: [{
	      name: "show",
	      rawName: "v-show",
	      value: (_vm.orders.length == 0),
	      expression: "orders.length == 0"
	    }],
	    attrs: {
	      "message": "您还没有相关订单"
	    }
	  }), _vm._v(" "), _vm._l((_vm.orders), function(o) {
	    return _c('div', {
	      staticClass: "orders_content"
	    }, [_c('router-link', {
	      staticClass: "orders_code",
	      attrs: {
	        "to": "/orders_detail",
	        "tag": "div"
	      }
	    }, [_c('span', [_vm._v("订单号：" + _vm._s(o.orderCode))]), _vm._v(" "), (o.orderState == 1) ? _c('span', {
	      staticClass: "order_state"
	    }, [_vm._v("已完成")]) : _vm._e(), _vm._v(" "), (o.orderState == 2) ? _c('span', {
	      staticClass: "order_state"
	    }, [_vm._v("待付款")]) : _vm._e(), _vm._v(" "), (o.orderState == 3) ? _c('span', {
	      staticClass: "order_state"
	    }, [_vm._v("待发货")]) : _vm._e(), _vm._v(" "), (o.orderState == 4) ? _c('span', {
	      staticClass: "order_state"
	    }, [_vm._v("待收货")]) : _vm._e(), _vm._v(" "), (o.orderState == 5) ? _c('span', {
	      staticClass: "order_state"
	    }, [_vm._v("待评价")]) : _vm._e()]), _vm._v(" "), _c('router-link', {
	      staticClass: "orders_commodity",
	      attrs: {
	        "to": "/orders_detail",
	        "tag": "div"
	      }
	    }, _vm._l((o.commodityList), function(i) {
	      return _c('img', {
	        attrs: {
	          "src": i.commodityUrl
	        }
	      })
	    })), _vm._v(" "), _vm._m(0, true)], 1)
	  })], 2)
	},staticRenderFns: [function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
	  return _c('div', {
	    staticClass: "orders_operation"
	  }, [_c('a', [_vm._v("取消订单")]), _vm._v(" "), _c('a', [_vm._v("支付订单")])])
	}]}
	if (false) {
	  module.hot.accept()
	  if (module.hot.data) {
	     require("vue-loader/node_modules/vue-hot-reload-api").rerender("data-v-fe0602da", module.exports)
	  }
	}

/***/ },

/***/ 198:
/***/ function(module, exports, __webpack_require__) {

	var __vue_exports__, __vue_options__
	var __vue_styles__ = {}

	/* styles */
	__webpack_require__(199)

	/* script */
	__vue_exports__ = __webpack_require__(201)

	/* template */
	var __vue_template__ = __webpack_require__(202)
	__vue_options__ = __vue_exports__ = __vue_exports__ || {}
	if (
	  typeof __vue_exports__.default === "object" ||
	  typeof __vue_exports__.default === "function"
	) {
	if (Object.keys(__vue_exports__).some(function (key) { return key !== "default" && key !== "__esModule" })) {console.error("named exports are not supported in *.vue files.")}
	__vue_options__ = __vue_exports__ = __vue_exports__.default
	}
	if (typeof __vue_options__ === "function") {
	  __vue_options__ = __vue_options__.options
	}
	__vue_options__.__file = "/Users/lvyang/repositories/node_web/views/orders_detail.vue"
	__vue_options__.render = __vue_template__.render
	__vue_options__.staticRenderFns = __vue_template__.staticRenderFns
	__vue_options__._scopeId = "data-v-b56238a6"

	/* hot reload */
	if (false) {(function () {
	  var hotAPI = require("vue-loader/node_modules/vue-hot-reload-api")
	  hotAPI.install(require("vue"), false)
	  if (!hotAPI.compatible) return
	  module.hot.accept()
	  if (!module.hot.data) {
	    hotAPI.createRecord("data-v-b56238a6", __vue_options__)
	  } else {
	    hotAPI.reload("data-v-b56238a6", __vue_options__)
	  }
	})()}
	if (__vue_options__.functional) {console.error("[vue-loader] orders_detail.vue: functional components are not supported and should be defined in plain js files using render functions.")}

	module.exports = __vue_exports__


/***/ },

/***/ 199:
/***/ function(module, exports, __webpack_require__) {

	// style-loader: Adds some css to the DOM by adding a <style> tag

	// load the styles
	var content = __webpack_require__(200);
	if(typeof content === 'string') content = [[module.id, content, '']];
	// add the styles to the DOM
	var update = __webpack_require__(9)(content, {});
	if(content.locals) module.exports = content.locals;
	// Hot Module Replacement
	if(false) {
		// When the styles change, update the <style> tags
		if(!content.locals) {
			module.hot.accept("!!../node_modules/css-loader/index.js!../node_modules/vue-loader/lib/style-rewriter.js?id=data-v-b56238a6&scoped=true!../node_modules/vue-loader/lib/selector.js?type=styles&index=0!./orders_detail.vue", function() {
				var newContent = require("!!../node_modules/css-loader/index.js!../node_modules/vue-loader/lib/style-rewriter.js?id=data-v-b56238a6&scoped=true!../node_modules/vue-loader/lib/selector.js?type=styles&index=0!./orders_detail.vue");
				if(typeof newContent === 'string') newContent = [[module.id, newContent, '']];
				update(newContent);
			});
		}
		// When the module is disposed, remove the <style> tags
		module.hot.dispose(function() { update(); });
	}

/***/ },

/***/ 200:
/***/ function(module, exports, __webpack_require__) {

	exports = module.exports = __webpack_require__(8)();
	// imports


	// module
	exports.push([module.id, "\n.re_buy[data-v-b56238a6]{\n\tposition: fixed;\n\tbottom: 0;\n\theight: 1.35rem;\n\twidth: 10rem;\n\tbackground-color: #ffffff;\n}\n.re_buy > div[data-v-b56238a6]{\n\tfloat: right;\n\tmargin-top: 0.3rem;\n\tmargin-right: 0.4rem;\n\twidth: 2rem;\n\theight: 0.8rem;\n\tline-height: 0.8rem;\n\ttext-align: center;\n\tborder:solid 1px #d81e06;\n\tcolor: #d81e06 ;\n\tborder-radius: 5px;\n}\n.orders_num_detail[data-v-b56238a6]{\n\tbackground-color: #ffffff;\n\tmargin-top: 0.2rem;\n\tbox-sizing: border-box;\n\tpadding:0.2rem 0.4rem;\n\tcolor: #333333;\n}\n.orders_num_detail .commodity_num_price[data-v-b56238a6]{\n\tfont-size: 0.4rem;\n\theight: 0.8rem;\n\tline-height: 0.8rem;\n}\n.orders_num_detail .freight[data-v-b56238a6]{\n\theight: 0.7rem;\n\tborder-bottom: solid 1px #dddddd;\n}\n.commodity_num_price a[data-v-b56238a6],.orders_num_detail .freight a[data-v-b56238a6]{\n\tdisplay: inline-block;\n\tfloat: right;\n\tcolor: #d81e06;\n}\n.orders_num_detail .payment[data-v-b56238a6],.orders_num_detail .order_time[data-v-b56238a6]{\n\ttext-align: right;\n}\n.orders_num_detail .payment[data-v-b56238a6]{\n\tfont-size: 0.4rem;\n\theight: 0.8rem;\n\tline-height: 0.8rem;\n}\n.orders_num_detail .payment a[data-v-b56238a6]{\n\tfont-size: 0.45rem;\n\tcolor: #d81e06;\n}\n.orders_num_detail .order_time[data-v-b56238a6]{\n\tfont-size: 0.3rem;\n\tcolor: #c5c5c5;\n\theight: 0.6rem;\n\tline-height: 0.6rem;\n}\n.order_commodity[data-v-b56238a6]{\n\tbackground-color: #ffffff;\n\tborder-bottom: solid 1px #dddddd;\n\theight: 2.5rem;\n\tcolor: #333333;\n\tfont-size: 0.35rem;\n}\n.order_commodity img[data-v-b56238a6]{\n\twidth: 2.3rem;\n\theight: 2.3rem;\n\tmargin-top: 0.1rem;\n\tvertical-align: middle;\n}\n.order_commodity .commodity_name[data-v-b56238a6]{\n\twidth: 7.5rem;\n\tdisplay: inline-block;\n\tbox-sizing: border-box;\n\tpadding-left: 0.2rem;\n\tpadding-right: 0.2rem;\n\tvertical-align: middle;\n\tcolor: #333333;\n}\n.commodity_name > div[data-v-b56238a6]:nth-child(1){\n\toverflow:hidden; \n\ttext-overflow:ellipsis;\n\t-webkit-box-orient:vertical;\n\t-webkit-line-clamp:2;\n}\n.commodity_name > div[data-v-b56238a6]:nth-child(2){\n\tmargin-top: 0.2rem;\n}\n.commodity_name a[data-v-b56238a6]{\n\tcolor: #d81e06;\n\tpadding-right: 0.2rem;\n\tpadding-left: 0.1rem;\n}\n.receipt_address[data-v-b56238a6]{\n\tbackground:#ffffff;\n\tpadding-bottom: 0.35rem;\n\twidth: 10rem;\n\tfont-size: 0.35rem;\n\tmargin-top: 0.2rem;\n\tmargin-bottom: 0.2rem;\n}\n.receipt_address .name[data-v-b56238a6]{\n\theight: 1rem;\n\tline-height: 1rem;\n\tbox-sizing: border-box;\n\tpadding-left: 1rem;\n\tcolor: #333333;\n}\n.receipt_address .name span[data-v-b56238a6]:nth-child(1){\n\tpadding-right: 1rem;\n}\n.receipt_address .address[data-v-b56238a6]{\n\tbackground: url(" + __webpack_require__(32) + ") 0.4rem center no-repeat;\n\tbackground-size: 0.45rem auto;\n\tbox-sizing: border-box;\n\tpadding-left: 1rem;\n\tcolor: #999999;\n\tpadding-right: 0.5rem;\n\t\n\toverflow:hidden; \n\ttext-overflow:ellipsis;\n\tdisplay:-webkit-box; \n\t-webkit-box-orient:vertical;\n\t-webkit-line-clamp:2;\n}\n.orders_detail[data-v-b56238a6]{\n\tbackground-color: #ffffff;\n\theight: 1rem;\n\tline-height: 1rem;\n\tbox-sizing: border-box;\n\tpadding-left: 0.4rem;\n\tpadding-right: 0.4rem;\n\tfont-size: 0.35rem;\n\tborder-bottom: solid 1px #dddddd;\n}\n.orders_detail .order_state[data-v-b56238a6]{\n\tdisplay: inline-block;\n\tfloat: right;\n\tcolor: #d81e06;\n}\n", ""]);

	// exports


/***/ },

/***/ 201:
/***/ function(module, exports) {

	"use strict";

	Object.defineProperty(exports, "__esModule", {
		value: true
	});
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//
	//

	exports.default = {
		data: function data() {
			return {
				orderCode: "123456789",
				orderState: "2",
				orderPrice: "100.00",
				orderTime: "2017-02-25 19:12:56",
				address: {
					id: "123",
					name: "吕扬",
					phone: "134****3525",
					areaAddress: "北京市海淀区",
					detailAddress: "14号楼7单元702室14号楼7单元702室14号楼7单元702室"
				},
				commodity: [{
					commodityName: "山西特产宁化府益源庆名醋山西特产宁化府益源庆名醋2400ml（31.8元）",
					commodityUrl: "img/logo.jpg",
					commodityNum: 1,
					commodityPrice: 52.48
				}, {
					commodityName: "山西特产宁化府益源庆名醋山西特产宁化府益源庆名醋2400ml（31.8元）",
					commodityUrl: "img/logo.jpg",
					commodityNum: 1,
					commodityPrice: 52.48
				}]
			};
		}
	};

/***/ },

/***/ 202:
/***/ function(module, exports, __webpack_require__) {

	module.exports={render:function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
	  return _c('div', [_c('div', {
	    staticClass: "orders_detail"
	  }, [_c('span', [_vm._v("订单号：" + _vm._s(_vm.orderCode))]), _vm._v(" "), (_vm.orderState == 1) ? _c('span', {
	    staticClass: "order_state"
	  }, [_vm._v("已完成")]) : _vm._e(), _vm._v(" "), (_vm.orderState == 2) ? _c('span', {
	    staticClass: "order_state"
	  }, [_vm._v("待付款")]) : _vm._e(), _vm._v(" "), (_vm.orderState == 3) ? _c('span', {
	    staticClass: "order_state"
	  }, [_vm._v("待发货")]) : _vm._e(), _vm._v(" "), (_vm.orderState == 4) ? _c('span', {
	    staticClass: "order_state"
	  }, [_vm._v("待收货")]) : _vm._e(), _vm._v(" "), (_vm.orderState == 5) ? _c('span', {
	    staticClass: "order_state"
	  }, [_vm._v("待评价")]) : _vm._e()]), _vm._v(" "), _c('div', {
	    staticClass: "receipt_address"
	  }, [_c('div', {
	    staticClass: "name"
	  }, [_c('span', [_vm._v(_vm._s(_vm.address.name))]), _c('span', [_vm._v(_vm._s(_vm.address.phone))])]), _vm._v(" "), _c('div', {
	    staticClass: "address"
	  }, [_vm._v(_vm._s(_vm.address.areaAddress) + _vm._s(_vm.address.detailAddress))])]), _vm._v(" "), _vm._l((_vm.commodity), function(c) {
	    return _c('div', {
	      staticClass: "order_commodity"
	    }, [_c('img', {
	      attrs: {
	        "src": c.commodityUrl
	      }
	    }), _vm._v(" "), _c('div', {
	      staticClass: "commodity_name"
	    }, [_c('div', [_vm._v(_vm._s(c.commodityName))]), _vm._v(" "), _c('div', [_vm._v("数量："), _c('a', [_vm._v(_vm._s(c.commodityNum))]), _vm._v("价格："), _c('a', [_vm._v("¥ " + _vm._s(c.commodityPrice))])])])])
	  }), _vm._v(" "), _c('div', {
	    staticClass: "orders_num_detail"
	  }, [_c('div', {
	    staticClass: "commodity_num_price"
	  }, [_vm._v("商品总额"), _c('a', [_vm._v("¥ " + _vm._s(_vm.orderPrice))])]), _vm._v(" "), _vm._m(0), _vm._v(" "), _c('div', {
	    staticClass: "payment"
	  }, [_vm._v("实付款："), _c('a', [_vm._v("¥ " + _vm._s(_vm.orderPrice))])]), _vm._v(" "), _c('div', {
	    staticClass: "order_time"
	  }, [_vm._v("下单时间：" + _vm._s(_vm.orderTime))])]), _vm._v(" "), _vm._m(1)], 2)
	},staticRenderFns: [function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
	  return _c('div', {
	    staticClass: "freight"
	  }, [_vm._v("+ 运费"), _c('a', [_vm._v("¥ 0.00")])])
	},function (){var _vm=this;var _h=_vm.$createElement;var _c=_vm._self._c||_h;
	  return _c('div', {
	    staticClass: "re_buy"
	  }, [_c('div', [_vm._v("再次购买")])])
	}]}
	if (false) {
	  module.hot.accept()
	  if (module.hot.data) {
	     require("vue-loader/node_modules/vue-hot-reload-api").rerender("data-v-b56238a6", module.exports)
	  }
	}

/***/ }

});